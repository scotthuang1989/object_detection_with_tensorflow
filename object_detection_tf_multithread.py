...
arg_parser.add_argument('-t', "--thread", type=int, default=1,
...
processed_q = queue_seq.Queue_Seq(maxsize=200, thread=True)
...
def image_worker(image_q, video_file):
    ...
    while ret:
        if stop_event.is_set():
            logging.info("image worker exit")
            break
...
def main():
    ...
    fps = fps_measure.FPS()
    fps.start_count()
    detector_process = [threading.Thread(target=object_detection_worker,
